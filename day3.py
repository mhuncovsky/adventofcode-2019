...
def trace_wire_path(string):
    moves = ((move[0], int(move[1:])) for move in string.split(','))
    ...
    for direction, distance in moves:
        for _ in range(distance):
            if direction == 'R':
                ...
                position[1] -= 1
            steps += 1
            goes_through.append((tuple(position), steps))
    return goes_through

def get_sorted_intersections(path_a, path_b):
    points_a = set(path_a)
    points_b = set(path_b)
    diff = points_a.intersection(points_b)
    return sorted(diff, key=lambda x: get_distance((0, 0), x))

def solve(wire_a, wire_b):
    path_a, steps_a = zip(*trace_wire_path(wire_a))
    path_b, steps_b = zip(*trace_wire_path(wire_b))

    intersections = get_sorted_intersections(path_a, path_b)

    steps = max(steps_a) + max(steps_b)
    for point in intersections:
        steps = min(steps, steps_a[path_a.index(point)] + steps_b[path_b.index(point)])
    
    closest_intersection = intersections[0]
    
    return get_distance((0, 0), closest_intersection), steps
...
d, s = solve(wire_a, wire_b)
print(f'MIN DISTANCE TO INTERSECTION: {d}')
print(f'MIN STEPS TO INTERSECTION:    {s}')
